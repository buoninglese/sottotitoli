...
try {
  ...
  db.pragma('cache_size = -64000'); // 64MB cache — the DB is only 20MB, so it all fits
  console.log('[cefr] DB opened:', DB_PATH);
} catch (err) {
  ...
  // Don't crash the server — routes will return 503
}
